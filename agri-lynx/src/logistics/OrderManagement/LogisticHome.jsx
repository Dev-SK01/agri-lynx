...
const LogisticHome = () => {
  const { LogisticOrders, LogisticData, acceptOrder, deleteOrder } =
  ...
  async function handleUpdateBookingStatus(orderId, action) {
    try {
      const req = await fetch(
        import.meta.env.VITE_API_BASE_URL + `/logistic/updatebookingstatus`,
        {
          method: "POST",
          headers: {
  ...
  if (window.location.pathname === "/logisticdashboard") {
        ...
        >
          <div>
            ...
            <h1 className="flex justify-center items-center text-center rounded-xl h-20 pt-0 bg-[var(--primary)] mt-2 ms-2 w-60 font-bold font-inter">
              {order.farmer.address}, {order.farmer.village},{" "}
              {order.farmer.taluk}
            </h1>
          </div>
